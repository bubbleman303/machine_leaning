...
import random
from src.othello.src.conf import config


class OthelloAgent(Board):
    def __init__(self):
        super().__init__()
        self.all_read = 10
        self.blank_num = None
    ...
    def get_max_pos_from_read_his(self, board, turn, return_list=False):
        ll = []
    ...
    def get_stone_count_of_agent(self, board):
        black_count = len(np.where(board == self.black)[0])
        white_count = len(np.where(board == self.white)[0])
    ...
    def agent_put(self, board, turn, pss):
        ...
        if blank_num <= self.all_read:
            ...
            pos, self.max_count = self.get_max_pos_from_read_his(board, turn)
            self.winner_str = "引き分け"
            if self.max_count > 0:
                self.winner_str = "黒の勝ち"
            elif self.max_count < 0:
                self.winner_str = "白の勝ち"
            print(f"{self.winner_str}:{self.max_count}")
            return pos
